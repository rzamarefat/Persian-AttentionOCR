...
class OCR(nn.Module):
    def __init__(self, img_width, img_height, nh, n_classes, max_len, SOS_token, EOS_token):
        ...
        # summary(self.backbone.to("cuda"),input_size=(3, img_height, img_width))
                
        self.__return_nodes = {
            'layer4.2.relu_2': 'layer4',
        }
        self.backbone = create_feature_extractor(resnet50(), return_nodes=self.__return_nodes)
        ...
        f = self.backbone(torch.rand(1, 3, img_height, img_width))["layer4"]
        ...
        self.encode_emb = nn.Linear(2048 + self._fh + self._fw, nh)
    ...
    def forward(self, input_, target_seq=None, teacher_forcing_ratio=0):
        ...
        encoder_outputs = self.backbone(input_)["layer4"]
        b, fc, fh, fw = encoder_outputs.size()
        # print(b, fc, fh, fw)
        ...
        encoder_outputs = encoder_outputs.contiguous().view(b, -1, 2048 + self._fh + self._fw)
